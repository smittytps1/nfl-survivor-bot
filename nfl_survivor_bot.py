...
# --- 1. SEARCH-GROUNDED DEEP AI REASONING (GEMINI) ---
def generate_deep_ai_reasoning(weekly_slates, optimal_path):
    ...
    for w in active_weeks:
        ...
        1. DO NOT mention the point spread, spread number, or betting line number anywhere in your text.
        2. NEVER use repetitive boilerplate phrases (e.g., do NOT say 'clean baseline', 'roster continuity', 'crowd cadence', or 'home field operational familiarity').
        3. CITE SPECIFIC SCHEMATICS & MATCHUP TRAITS for these exact teams:
           ...
           - Efficiency & EPA: Net passing vs rushing EPA differentials, explosive play rate, 3rd-down success rate, and red zone conversion.
           - Situational & Environmental: Dome track speed vs outdoor weather/wind, international travel (e.g. Melbourne opener), or West Coast body-clock travel fatigue in early 1:00 PM ET starts.
        4. If 'is_recommended' is true: Explicitly explain why picking this specific team optimizes 18-week Survivor path equity while preserving elite Future Value (FV) assets (KC, BUF, BAL, DET) for late-season bottlenecks.
        ...
        try:
            ...
            print(f"Notice during AI generation for Week {w}: {e}. Retrying individual game queries...")
            for c in week_candidates:
                if (w, c["team"]) not in reasoning_map:
                    single_prompt = f"""
                    Provide a unique, 35-word quantitative NFL analysis for {c['team']} vs {c['opponent']} in Week {w} (is_home={c['is_home']}, is_survivor_pick={c['is_recommended']}).
                    Do not mention the betting line numbers. Synthesize EPA/play differentials, Pass Block Win Rate vs pass rush, red zone efficiency, and Survivor Future Value preservation.
                    """
                    try:
                        resp = client.models.generate_content(model="gemini-2.5-flash", contents=single_prompt)
                        reasoning_map[(w, c["team"])] = resp.text.strip()
                    except Exception:
...
def fetch_online_schedule():
    ...
    # Official 2026 NFL Regular Season Kickoff Slate
...
# --- 3. SURVIVOR OPTIMIZER (HUNGARIAN BIPARTITE MATCHING) ---
...
def sync_to_google_sheets():
    ...
    # 5. Generate Multi-Factor Synthesis Reasoning
