...
def generate_deep_ai_reasoning(weekly_slates, optimal_path):
  ...
  active_weeks = [
      w
      for w, sl in weekly_slates.items()
      if any(c.get("spread") is not None for c in sl)
  ]
...
def sync_to_google_sheets():
  ...
  print(
      f"Detected {len(locked_picks)} user locked picks in Column D:"
      f" {locked_picks}"
  )
  ...
  headers = [
      ...
      "Market Win %",
      "Model Win %",
      "Reasoning & Multi-Factor Synthesis",
  ]
