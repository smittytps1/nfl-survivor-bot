...
def calculate_model_prob(
    market_prob: float, is_home: bool, spread: float
) -> float:
...
def build_candidates_for_week(games, live_odds_map, espn_odds_map, week):
  ...
  candidates.sort(
      key=lambda x: (
          x["mod_prob"] is not None,
          x["mod_prob"] if x["mod_prob"] is not None else 0,
      ),
      reverse=True,
  )
  return candidates[:5]
...
# --- 2. SURVIVOR OPTIMIZER (HUNGARIAN BIPARTITE MATCHING) ---
...
def sync_to_google_sheets():
  ...
  for w in range(1, WEEKS + 1):
    ...
    all_weekly_slates[w] = build_candidates_for_week(
        schedule[w], live_odds_map, espn_odds, w
    )

  # 4. Dynamically re-optimize path around user locks
  optimal_path = solve_survivor_path(all_weekly_slates, locked_picks)

  # 5. Calculate Cumulative Survival Percentage
  cum_prob = 1.0
  for w in range(1, WEEKS + 1):
    ...
    matched = next(
        (
            c
            for c in week_cands
            if c["team"] == effective_team and c["mod_prob"] is not None
        ),
        None,
    )
  ...
  # 6. Construct 9-Column Matrix (A through I)
  headers = [
      "Week",
      "Recommended Pick",
      "|",
  ...
  # Columns E-I (Merged headers and 5 candidate rows per week)
  for w in range(1, WEEKS + 1):
    rec_team = optimal_path.get(w, "")
    ...
    for i in range(5):
      ...
      if i < len(cands):
        ...
        team_display = (
            f"**{cand['team']}**" if cand.get("home", False) else cand["team"]
        )
        spread_display = (
            f"{cand['spread']:+.1f}" if cand["spread"] is not None else ""
        )
        m_prob_display = (
            f"{cand['m_prob'] * 100:.1f}%"
            if cand["m_prob"] is not None
            else ""
        )
        mod_prob_display = (
            f"{cand['mod_prob'] * 100:.1f}%"
            if cand["mod_prob"] is not None
            else ""
        )
  ...
  # 7. Write matrix to Google Sheet
  ...
  for rng in merge_ranges:
    batch_formats.append({
        ...
        "format": {
            ...
            "textFormat": {
                "bold": True,
                "foregroundColor": {"red": 0.05, "green": 0.16, "blue": 0.28},
            },
            "horizontalAlignment": "CENTER",
        },
    })
  ...
  print("Success: Google Sheet cleanly updated without reasoning column.")
